...
from vinylkit.naming import generate_path, move_file
from vinylkit.tagging import scan_folder, tag_audio_file, write_release_info
...
@click.option("--dry-run", is_flag=True, help="Display changes without writing to files.")
@click.option("--no-artwork", is_flag=True, help="Disable artwork embedding.")
@click.option("--rename/--no-rename", "do_rename", default=None, help="Automatically rename and move files after tagging.")
@click.option("--library-root", "lib_root_override", type=click.Path(file_okay=False, dir_okay=True, path_type=Path), help="Override the library root directory for renaming.")
@click.option("--merge", is_flag=True, help="Preserve existing tags (default is to clear and replace).")
@click.pass_obj
def tag(config: AppConfig, paths: tuple[Path, ...], release_id: int | None, query: str | None, dry_run: bool, no_artwork: bool, do_rename: bool | None, lib_root_override: Path | None, merge: bool) -> None:
    ...
    if not paths:
        if config.recordings_root:
            ...
            raise click.UsageError("No PATH provided and 'recordings_root' is not configured.")
    ...
    if not release_id and not query and len(paths) > 1:
        console.print("[yellow]Batch mode: You will be prompted for each folder.[/yellow]")        
    ...
    for path in paths:
        ...
        try:
            if not current_release_id and not current_query:
                current_query = click.prompt(f"Enter search query or Release ID for {path.name}")  
            ...
            if current_query:
                ...
                if not results:
                    console.print(f"[yellow]No results found for query: {current_query}[/yellow]") 
                    continue
                ...
                for i, res in enumerate(results[:10], 1):
                    ...
                    console.print(f"{i}. [cyan]{title}[/cyan] ({year}, {country}, {fmt})")
            ...
            console.print(f"Loaded Release: [bold]{', '.join(release.artists)} - {release.title}[/bold] ({release.year})")

            # Artwork handling
            artwork_data = None
            if not no_artwork and release.images:
                primary_image = next((i for i in release.images if i.type == "primary"), release.images[0])
                try:
                    with console.status("[bold green]Downloading artwork...") as status:
                        artwork_data = client.download_image(primary_image.resource_url)
                except Exception as e:
                    console.print(f"[yellow]Warning: Failed to download artwork: {e}[/yellow]")    
            ...
            if not audio_files:
                console.print(f"[yellow]No supported audio files (MP3/FLAC) found in {path}.[/yellow]")
                continue

            if len(audio_files) != len(release.tracklist):
                console.print(f"[yellow]Warning: Found {len(audio_files)} files but release has {len(release.tracklist)} tracks.[/yellow]")
                ...
                for i, file_path in enumerate(audio_files):
                    ...
                    if config.backup_enabled and config.backup_dir and not dry_run:
                        try:
                            ...
                            console.print(f"[yellow]Warning: Failed to backup {file_path.name}: {e}[/yellow]")

                    tag_audio_file(file_path, release, i, dry_run=dry_run, artwork_data=artwork_data, tag_mode=tag_mode)
                    tagged_paths.append(file_path)

            # Create info file
            if not dry_run:
                write_release_info(path, release)
            ...
            if do_rename:
                ...
                for i, source in enumerate(tagged_paths):
                    target = generate_path(lib_root, config.naming_pattern, release, i, source.suffix)
                    ...
                    console.print(f"[cyan]{source.name}[/cyan] -> [green]{display_target}[/green]")

                # Also move release_info.txt if it exists
                info_file = path / "release_info.txt"
                if info_file.exists() and moves:
                    info_target = moves[0][1].parent / "release_info.txt"
                    if info_file != info_target:
                        ...
                        console.print(f"[cyan]{info_file.name}[/cyan] -> [green]{display_info_target}[/green]")
...
@click.option("--commit", is_flag=True, help="Actually move the files (default is dry-run).")      
@click.option("--library-root", "lib_root_override", type=click.Path(file_okay=False, dir_okay=True, path_type=Path), help="Override the library root directory for renaming.")
@click.pass_obj
def rename(config: AppConfig, paths: tuple[Path, ...], release_id: int | None, commit: bool, lib_root_override: Path | None) -> None:
    ...
    for path in paths:
        ...
        try:
            if not current_release_id:
                current_release_id = click.prompt(f"Enter Discogs Release ID for {path.name}", type=int)
...
def login(config: AppConfig) -> None:
    ...
    if not config.consumer_key or config.consumer_key == DEFAULT_CONSUMER_KEY:
        console.print("[bold red]Error:[/bold red] You must set your own [bold]consumer_key[/bold] and [bold]consumer_secret[/bold] before logging in.")
        console.print("See the [bold]Authentication Guide (auth.md)[/bold] for instructions.")     
        return
    ...
    try:
        ...
        console.print("[bold green]Success![/bold green] You are now authenticated with Discogs.") 
...
def config_show(config_obj: AppConfig) -> None:
    ...
    console.print(f"[bold]Recordings Root:[/bold] {config_obj.recordings_root or 'Not Set'}")
    ...
    console.print(f"[bold]Consumer Key:[/bold] {'****' if config_obj.consumer_key else 'Not Set'}")
    ...
    console.print(f"[bold]Discogs Token:[/bold] {'****' if config_obj.discogs_token else 'Not Set'}")
