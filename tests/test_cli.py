...
def test_tag_interactive_search_options(runner: CliRunner, tmp_path: Path, mocker) -> None:
    ...
    mock_inst.search_releases.return_value = [
        {"id": 123, "title": "Test Title", "year": 2000, "country": "US", "format": ["Vinyl"]}
    ]
...
def test_tag_auto_move_skips_confirm(runner: CliRunner, tmp_path: Path, mocker) -> None:
    ...
    mock_release = DiscogsRelease(
        id=123, artists=["Artist"], title="Title", year=2000, 
    ...
    result = runner.invoke(cli, ["tag", str(tmp_path), "--id", "123", "--auto-move", "--library-root", str(tmp_path / "lib"), "--rename"])
...
def test_tag_filtered_search_real_collection_example(runner: CliRunner, tmp_path: Path, mocker) -> None:
    ...
    from vinylkit.models import DiscogsRelease, TrackInfo
    mock_release = DiscogsRelease(
        id=61232, artists=["Faithless"], title="Insomnia", year=1997,
        tracklist=[TrackInfo(position="A1", title="Insomnia (Monster Mix)")],
        labels=[], companies=[], formats=[], identifiers=[], extraartists=[],
        genres=["Electronic"], styles=["House"], notes="", images=[], uri=""
    )
    mock_client.get_release.return_value = mock_release
    ...
    result = runner.invoke(
        cli, 
        ["tag", str(tmp_path), "--artist", "Faithless", "--album", "Insomnia", "--rename", "--auto-move"],
        input="1\n"
    )
