...
def create_mock_release(rid: int, artist: str, title: str) -> DiscogsRelease:
    return DiscogsRelease(
        id=rid, artists=[artist], title=title, year=2000,
        tracklist=[TrackInfo(position="A1", title="Track 1")],
        labels=[], companies=[], formats=[], identifiers=[], extraartists=[],
        genres=[], styles=[], notes="", images=[], uri=""
    )
...
def test_ex_1_1_basic_id_tagging(runner, tmp_path, mock_discogs):
    ...
    mock_discogs.get_release.return_value = create_mock_release(19983, "Green Velvet", "Flash")
...
def test_ex_1_2_id_rename_automove(runner, tmp_path, mock_discogs):
    ...
    mock_discogs.get_release.return_value = create_mock_release(53088, "The Prodigy", "Wind It Up")
    result = runner.invoke(cli, [
        "tag", str(tmp_path), "--id", "53088", "--rename", "--auto-move", 
...
def test_ex_2_1_filtered_search_artist_album(runner, tmp_path, mock_discogs):
    ...
    (tmp_path / "01.mp3").write_text("audio")
    mock_discogs.search_releases.return_value = [{"id": 61232, "title": "Faithless - Insomnia", "year": 1997, "country": "UK", "format": ["Vinyl"]}]
    mock_discogs.get_release.return_value = create_mock_release(61232, "Faithless", "Insomnia")
    result = runner.invoke(cli, ["tag", str(tmp_path), "--artist", "Faithless", "--album", "Insomnia"], input="1\ny\n")
    assert result.exit_code == 0
...
def test_ex_2_2_filtered_search_multi_format(runner, tmp_path, mock_discogs):
    """Covers: vinylkit tag --artist 'The Chemical Brothers' --album 'Hey Boy Hey Girl' --format 'Vinyl, CD' """
    (tmp_path / "01.mp3").write_text("audio")
    mock_discogs.search_releases.return_value = [{"id": 4008, "title": "Chemical Brothers - Hey Boy Hey Girl", "year": 1999, "country": "UK", "format": ["Vinyl"]}]
    mock_discogs.get_release.return_value = create_mock_release(4008, "The Chemical Brothers", "Hey Boy Hey Girl")
    result = runner.invoke(cli, [
        "tag", str(tmp_path), "--artist", "The Chemical Brothers", "--album", "Hey Boy Hey Girl", "--format", "Vinyl, CD"
    ], input="1\ny\n")
    assert result.exit_code == 0
    mock_discogs.search_releases.assert_called_with(None, artist="The Chemical Brothers", album="Hey Boy Hey Girl", format=["Vinyl", "CD"])
...
def test_ex_3_1_global_search_catno(runner, tmp_path, mock_discogs):
    ...
    (tmp_path / "01.mp3").write_text("audio")
    mock_discogs.search_releases.return_value = [{"id": 20754, "title": "Plastikman - Sheet One", "year": 1993, "country": "US", "format": ["Vinyl"]}]
    mock_discogs.get_release.return_value = create_mock_release(20754, "Plastikman", "Sheet One")
    result = runner.invoke(cli, ["tag", str(tmp_path), "--search", "PLUS8028"], input="1\ny\n")
    assert result.exit_code == 0
...
def test_ex_3_2_global_search_artist_label(runner, tmp_path, mock_discogs):
    ...
    (tmp_path / "01.mp3").write_text("audio")
    mock_discogs.search_releases.return_value = [{"id": 165, "title": "Jeff Mills - Kat Moda EP", "year": 1997, "country": "US", "format": ["Vinyl"]}]
    mock_discogs.get_release.return_value = create_mock_release(165, "Jeff Mills", "Kat Moda EP")
    result = runner.invoke(cli, ["tag", str(tmp_path), "--search", "Jeff Mills Purpose Maker"], input="1\ny\n")
    assert result.exit_code == 0
...
def test_ex_4_1_inbox_scan(runner, tmp_path, mocker):
    ...
    mocker.patch("vinylkit.cli.load_config", return_value=AppConfig(library_root=tmp_path / "lib", recordings_root=inbox))
...
def test_ex_4_2_inbox_tag(runner, tmp_path, mock_discogs, mocker):
    ...
    (inbox / "track.mp3").write_text("data")
    mocker.patch("vinylkit.cli.load_config", return_value=AppConfig(library_root=tmp_path / "lib", recordings_root=inbox))
    mock_discogs.search_releases.return_value = [{"id": 57745, "title": "Underworld - Born Slippy", "year": 1996, "country": "UK", "format": ["Vinyl"]}]
    mock_discogs.get_release.return_value = create_mock_release(57745, "Underworld", "Born Slippy")
    result = runner.invoke(cli, ["tag", "--artist", "Underworld", "--album", "Born Slippy"], input="1\ny\n")
    assert result.exit_code == 0
...
def test_ex_4_3_dry_run(runner, tmp_path, mock_discogs, mocker):
    ...
    (tmp_path / "01.mp3").write_text("audio")
    mock_discogs.get_release.return_value = create_mock_release(1480380, "Massive Attack", "Unfinished Sympathy")
    spy = mocker.patch("vinylkit.cli.tag_audio_file")
    result = runner.invoke(cli, ["tag", str(tmp_path), "--id", "1480380", "--rename", "--dry-run"])
    assert result.exit_code == 0
...
def test_ex_4_4_batch_processing(runner, tmp_path, mock_discogs):
    ...
    f1 = tmp_path / "a1"; f1.mkdir()
    f2 = tmp_path / "a2"; f2.mkdir()
    (f1 / "t.mp3").write_text("a"); (f2 / "t.mp3").write_text("a")
    mock_discogs.get_release.side_effect = [create_mock_release(1, "A", "T"), create_mock_release(2, "A", "T")]
    result = runner.invoke(cli, ["tag", str(f1), str(f2), "--rename"], input="1\ny\n2\ny\n")
    assert result.exit_code == 0
...
def test_ex_6_1_library_root_override(runner, tmp_path, mock_discogs):
    ...
    (tmp_path / "01.mp3").write_text("audio")
    mock_discogs.get_release.return_value = create_mock_release(236605, "Daft Punk", "Homework")
    lib_over = tmp_path / "manual_lib"
    result = runner.invoke(cli, ["tag", str(tmp_path), "--id", "236605", "--rename", "--auto-move", "--library-root", str(lib_over)])
    assert result.exit_code == 0

def test_ex_6_2_merge_mode(runner, tmp_path, mock_discogs, mocker):
    """Covers: vinylkit tag --id 28203 --merge"""
    (tmp_path / "01.mp3").write_text("audio")
    mock_discogs.get_release.return_value = create_mock_release(28203, "Satoshi Tomiie", "Love In Traffic")
    spy = mocker.patch("vinylkit.cli.tag_audio_file")
